...
class Raining {
  ...
  creatRainingSystem(){
    ...
    // 生成点
    let rainPoint = new THREE.Points(geometry, material);
    this.scene.add(rainPoint);

    // 渲染动效
    this.thing = setInterval(() => {
      if (rainPoint) {
        const positions = rainPoint.geometry.getAttribute("position").array;
        // 遍历y
        ...
        rainPoint.geometry.getAttribute("position").needsUpdate = true;
      }
      // console.log('rainPoint',rainPoint);
    }, 30);
  }
}
